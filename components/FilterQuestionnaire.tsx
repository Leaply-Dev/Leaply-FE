...
import { Filter, X, ChevronDown, ChevronUp, MapPin, DollarSign, BookOpen, RotateCcw } from 'lucide-react';
...
export function FilterQuestionnaire({
  ...
  const [activeSection, setActiveSection] = useState<'region' | 'major' | 'tuition' | null>('region');
  ...
  const hasActiveFilters = filters.regions.length > 0 || filters.majors.length > 0 || filters.tuitionRange !== null;
    ...
    <div className={cn('bg-white rounded-xl border-2 border-gray-200 shadow-sm', className)}>
      ...
      <div className={cn('p-4 space-y-6', !isExpanded && 'hidden md:block')}>
        ...
        <div className="space-y-4">
                  ...
                  {availableRegions.map((region) => (
                    ...
                    >
                      ...
                      {region.count !== undefined && (
                        <span className="ml-1.5 text-xs opacity-75">({region.count})</span>
                      )}
                    </button>
                  ))}
                </div>
          ...
          <div className="space-y-3">
            <button
              onClick={() => setActiveSection(activeSection === 'tuition' ? null : 'tuition')}
              className="w-full flex items-center justify-between text-left group"
            >
              <div className="flex items-center gap-2">
              ...
              {filters.tuitionRange && (
                <Badge variant="outline" className="text-xs">
                  ${filters.tuitionRange.min.toLocaleString()} - ${filters.tuitionRange.max.toLocaleString()}
                </Badge>
              )}
            </button>
              ...
              <div className="pl-6 space-y-2 animate-in slide-in-from-top-2">
                <div className="grid grid-cols-2 gap-2">
                  {tuitionRanges.map((range) => {
                    const isSelected = filters.tuitionRange?.min === range.min && filters.tuitionRange?.max === range.max;
                    return (
                      <button
                        key={range.value}
                        onClick={() => setTuitionRange(isSelected ? null : { min: range.min, max: range.max })}
                        className={cn(
          ...
          <div className="space-y-3">
            <button
              onClick={() => setActiveSection(activeSection === 'major' ? null : 'major')}
              className="w-full flex items-center justify-between text-left group"
            >
              <div className="flex items-center gap-2">
