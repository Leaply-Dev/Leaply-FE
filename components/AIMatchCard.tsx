...
export function AIMatchCard({
	...
	return (
		<Card
			className={cn(
				"overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-2 border-primary/20",
				className,
			)}
		>
				...
				<div className="relative h-48 bg-linear-to-br from-leaf-green/10 to-light-green/5">
					...
					{fitScore > 0 && (
						<div className="absolute bottom-4 left-4">
							<Badge
								className={cn(
									"bg-card border-2 font-semibold shadow-md",
				...
				<div className="p-6">
						...
						<Dialog open={isWhyDialogOpen} onOpenChange={setIsWhyDialogOpen}>
							...
							<DialogContent className="max-w-md">
								<DialogHeader>
									...
									<DialogDescription>{t("whySchoolDesc")}</DialogDescription>
								</DialogHeader>
								...
								<div className="space-y-4 py-4">
									<div className="flex items-center gap-3 p-3 bg-primary/5 rounded-lg border border-primary/20">
										<div className="shrink-0">
											<Badge
												className={cn("border-2", getFitScoreColor(fitScore))}
											>
												{fitScore}%
											</Badge>
										</div>
								...
								<div className="flex gap-2">
									<Button asChild type="button" className="flex-1">
										<Link href={`/explore/${id}`}>{t("viewDetails")}</Link>
									</Button>
									{onAskAI && (
										<Button
											variant="outline"
					...
					<div className="flex gap-2">
						<Button asChild type="button" size="sm" className="flex-1">
							<Link href={`/explore/${id}`}>{t("learnMore")}</Link>
						</Button>
						<Button
							type="button"
							size="sm"
						...
						>
							{saved ? `★ ${t("saved")}` : `+ ${t("addToDreamList")}`}
						</Button>
					</div>
