...
import { Target, DollarSign, Globe, Sparkles, RotateCcw, MessageCircle } from 'lucide-react';
...
export function PersonaLabProfile({ onAskLeafy }: PersonaLabProfileProps) {
  ...
  const [savedProfiles, setSavedProfiles] = useState<{ name: string; config: ProfileConfig }[]>([]);
  ...
  const generateRecommendations = (): University[] => {
    ...
    if (profile.region !== 'Any Region') {
      filtered = baseUniversities.filter((uni) => {
        if (profile.region === 'North America') return ['USA', 'Canada'].includes(uni.country);
        if (profile.region === 'Europe') return ['Germany', 'Netherlands', 'France'].includes(uni.country);
        if (profile.region === 'Asia-Pacific') return ['Singapore', 'Australia', 'Japan'].includes(uni.country);
        if (profile.region === 'UK & Ireland') return ['UK'].includes(uni.country);
        return true;
      });
    }
    ...
    return filtered.map((uni) => {
      ...
      if (profile.region !== 'Any Region' && uni.country) {
        matchReasons.push(`Matches your preferred region (${profile.region})`);
      }
