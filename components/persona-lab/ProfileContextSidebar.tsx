...
const PERSONA_SUMMARIES: Record<string, string> = {
	...
	complete_vi:
		"Một nhà nghiên cứu trẻ với tư duy hệ thống và đam mê giải quyết vấn đề môi trường. Kết hợp nền tảng khoa học vững chắc với khả năng lãnh đạo và xây dựng mạng lưới, bạn đã chứng minh cam kết tạo tác động xã hội thông qua các dự án như hệ thống giám sát nước IoT và Environmental Youth Network. Điểm mạnh nổi bật là khả năng kết nối công nghệ với nhu cầu thực tế của cộng đồng.",
	complete_en:
		"A young researcher with systems thinking and passion for solving environmental challenges. Combining a strong scientific foundation with leadership and network-building abilities, you have demonstrated commitment to social impact through projects like the IoT water monitoring system and the Environmental Youth Network. Your standout strength is connecting technology with real community needs.",
	analytical_vi:
		"Một nhà tư duy hệ thống với khả năng phân tích sắc bén, kết hợp nền tảng khoa học vững chắc với đam mê nghiên cứu giải pháp bền vững cho các thách thức xã hội.",
	analytical_en:
		"A systematic thinker with sharp analytical skills, combining a strong scientific foundation with a passion for researching sustainable solutions to social challenges.",
	creative_vi:
		"Người sáng tạo đa năng, luôn tìm kiếm giao điểm giữa nghệ thuật và công nghệ để tạo ra những trải nghiệm ý nghĩa và tác động tích cực đến cộng đồng.",
	creative_en:
		"A versatile creative, always seeking the intersection of art and technology to create meaningful experiences and positive community impact.",
	leadership_vi:
		"Nhà lãnh đạo trẻ với tầm nhìn toàn cầu, có khả năng kết nối con người và truyền cảm hứng thay đổi thông qua các dự án cộng đồng có tác động lâu dài.",
	leadership_en:
		"A young leader with global vision, capable of connecting people and inspiring change through community projects with lasting impact.",
};
...
export function ProfileContextSidebar() {
	...
	// State for expandable persona summary
	const [isPersonaSummaryExpanded, setIsPersonaSummaryExpanded] =
		useState(false);
	...
	const highlightedAchievement =
		keyStories.find((s) => s.isPinned) || keyStories[0];
	...
	const getPersonaSummary = () => {
		...
		const allTracksCompleted =
			completedTracks === tracks.length && tracks.length === 4;
	...
	return (
		<ScrollArea className="flex-1 h-full">
			<div className="p-6 space-y-6">
				...
				{/* Persona Summary - AI Generated (Collapsible) */}
				<div className="space-y-2">
					...
					<div className="relative">
						<p
							className={`text-sm text-foreground leading-relaxed pl-6 border-l-2 border-border transition-all ${
								!isPersonaSummaryExpanded ? "line-clamp-2" : ""
							}`}
						>
							{getPersonaSummary()}
						</p>
					...
					{getPersonaSummary().length > 100 && (
						<Button
							...
							size="sm"
							onClick={() =>
								setIsPersonaSummaryExpanded(!isPersonaSummaryExpanded)
							}
							className="h-6 text-xs text-muted-foreground hover:text-foreground p-0 ml-6"
						>
							{isPersonaSummaryExpanded ? (
								<>
				...
				{/* Target Countries */}
				{preferences?.targetCountries &&
					preferences.targetCountries.length > 0 && (
						<div className="space-y-3">
							<h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
								<Target className="w-4 h-4" />{" "}
								{t("personaLab", "targetCountries")}
							</h4>
							<div className="flex flex-wrap gap-2">
				...
				<div className="space-y-3 pt-4 border-t border-border">
					...
					<div className="space-y-2">
						<span className="text-xs text-muted-foreground">
							{t("personaLab", "certificates") || "Certificates"}
						</span>
