...
import { type Node, type Edge, useViewport } from "@xyflow/react";
import { TRACK_DIRECTIONS, CANVAS_CENTER, getNodeDimensions } from "@/lib/utils/canvasLayout";
...
export function useOrganicLayout({ nodes, edges, setNodes }: UseOrganicLayoutProps) {
    ...
    const forceCollideRect = useCallback(() => {
        ...
        force.initialize = (n: SimNode[]) => { simNodes = n; };
        return force;
    }, []);
    ...
    useEffect(() => {
        ...
        if (hasNewNodes || hasDeletedNodes || !simulationRef.current) {
            ...
            simulationRef.current = forceSimulation(simNodes as SimNode[])
                .force("link", forceLink(simEdges).id((d: any) => d.id).distance(180).strength(0.2))
                ...
                .force("radial", forceRadial((d: any) => {
                    ...
                    return 0;
                }, CANVAS_CENTER.x, CANVAS_CENTER.y).strength(0.6))
                // Theme Attractors (Gravity Wells)
                .force("x", forceX().x((d: any) => {
                    if (d.id === 'core') return CANVAS_CENTER.x;
                    const track = d.track;
                    if (track && TRACK_DIRECTIONS[track as keyof typeof TRACK_DIRECTIONS]) {
                        const angle = TRACK_DIRECTIONS[track as keyof typeof TRACK_DIRECTIONS].angle;
                        const dist = d.type === 'summary' ? 250 : 450;
                        return CANVAS_CENTER.x + Math.cos(angle * Math.PI / 180) * dist;
                    }
                ...
                .force("y", forceY().y((d: any) => {
                    ...
                    if (track && TRACK_DIRECTIONS[track as keyof typeof TRACK_DIRECTIONS]) {
                        const angle = TRACK_DIRECTIONS[track as keyof typeof TRACK_DIRECTIONS].angle;
                        const dist = d.type === 'summary' ? 250 : 450;
                        return CANVAS_CENTER.y + Math.sin(angle * Math.PI / 180) * dist;
                    }
