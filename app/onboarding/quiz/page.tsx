...
export default function OnboardingQuizPage() {
  ...
  const interests = ['Research', 'Entrepreneurship', 'Sports', 'Arts', 'Community Service', 'Technology'];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updatePreferences({
      ...
      campusSetting: formData.campusSetting as any,
  ...
  const toggleArrayItem = (field: 'preferredRegions' | 'languagesOfInstruction' | 'interests', value: string) => {
    setFormData(prev => ({
      ...prev,
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, desiredMajor: e.target.value }))}
                    required
                  />
                </div>
                    ...
                    {regions.map(region => (
                      <label key={region} className="flex items-center gap-2 cursor-pointer">
                        <Checkbox
                          ...
                          onChange={() => toggleArrayItem('preferredRegions', region)}
                        />
                ...
                <div className="space-y-2">
                  <Label htmlFor="campusSetting">Campus Setting Preference</Label>
                  <Select
                    id="campusSetting"
                    value={formData.campusSetting}
                    onChange={(e) => setFormData(prev => ({ ...prev, campusSetting: e.target.value }))}
                    required
                  >
                    <option value="">Select preference</option>
                    <option value="urban">Urban - City center location</option>
                    <option value="suburban">Suburban - Near city but quieter</option>
                    ...
                    {languages.map(language => (
                      <label key={language} className="flex items-center gap-2 cursor-pointer">
                        <Checkbox
                          ...
                          onChange={() => toggleArrayItem('languagesOfInstruction', language)}
                        />
